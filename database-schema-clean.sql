-- EduLink Database Schema - Clean Version
-- This version handles existing objects gracefully

-- Enable UUID extension
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";

-- Create custom types/enums (skip if they already exist)
DO $$ BEGIN
...
EXCEPTION
    WHEN duplicate_object THEN 
        RAISE NOTICE 'school_category type already exists, skipping...';
END $$;
...
EXCEPTION
    WHEN duplicate_object THEN 
        RAISE NOTICE 'request_type type already exists, skipping...';
END $$;
...
EXCEPTION
    WHEN duplicate_object THEN 
        RAISE NOTICE 'request_status type already exists, skipping...';
END $$;
...
EXCEPTION
    WHEN duplicate_object THEN 
        RAISE NOTICE 'interest_status type already exists, skipping...';
END $$;
...
CREATE INDEX IF NOT EXISTS idx_requests_school_id ON public.requests(school_id);
CREATE INDEX IF NOT EXISTS idx_requests_status ON public.requests(status);
CREATE INDEX IF NOT EXISTS idx_requests_type ON public.requests(type);
CREATE INDEX IF NOT EXISTS idx_updates_request_id ON public.updates(request_id);
CREATE INDEX IF NOT EXISTS idx_interests_donor_id ON public.interests(donor_id);
CREATE INDEX IF NOT EXISTS idx_interests_request_id ON public.interests(request_id);
...
-- Create trigger for requests table (drop first if exists)
...
DROP POLICY IF EXISTS "School admins can view their own schools" ON public.schools;
DROP POLICY IF EXISTS "School admins can insert their own schools" ON public.schools;
DROP POLICY IF EXISTS "School admins can update their own schools" ON public.schools;
...
DROP POLICY IF EXISTS "Anyone can view approved/listed requests" ON public.requests;
DROP POLICY IF EXISTS "School admins can view their own requests" ON public.requests;
DROP POLICY IF EXISTS "School admins can insert requests for their schools" ON public.requests;
DROP POLICY IF EXISTS "School admins can update their own requests" ON public.requests;
...
DROP POLICY IF EXISTS "Anyone can view updates for approved requests" ON public.updates;
DROP POLICY IF EXISTS "School admins can manage updates for their requests" ON public.updates;
...
-- Drop existing interest policies
DROP POLICY IF EXISTS "Donors can view their own interests" ON public.interests;
DROP POLICY IF EXISTS "Donors can insert their own interests" ON public.interests;
DROP POLICY IF EXISTS "Donors can update their own interests" ON public.interests;
DROP POLICY IF EXISTS "School admins can view interests in their requests" ON public.interests;
...
-- Success message
DO $$ BEGIN
    RAISE NOTICE '✅ EduLink database schema setup completed successfully!';
    RAISE NOTICE '📋 Created tables: users, schools, requests, updates, interests';
